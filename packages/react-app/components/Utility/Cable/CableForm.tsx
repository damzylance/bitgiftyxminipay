...
export const CableForm = (props: any) => {
	...
	const fee =
		userCountry === "NG"
			? parseFloat(process.env.NEXT_PUBLIC_TF as string)
			: userCountry === "KE"
			? 10
			: userCountry === "GH"
			? 1
	...
	const validateMeter = async (e: any) => {
		...
		const validate = await axios
			.get(
				`${process.env.NEXT_PUBLIC_BASE_URL}validate-bill-service/?item-code=${
					props.itemCode || itemCode
				}&biller-code=${props.cable}&customer=${customer}`
			)
			.then((response) => {
				return response;
			})
	...
	const buyCable = async (data: any) => {
		if (
			parseInt(data?.plan?.split(",")[1]) <
				parseFloat(tokenBalance) * tokenToNairaRate ||
			data.amount < parseFloat(tokenBalance) * tokenToNairaRate
		) {
			if (window.ethereum) {
				try {
					setLoading(true);
			...
			console.log(parseFloat(tokenBalance) * tokenToNairaRate);
