...
const Cable = (props: any) => {
	...
	const merchantsByCountry = merchants.find(
	...
	useEffect(() => {
		axios
			.get(
				`${process.env.NEXT_PUBLIC_BASE_URL}v2/get-bill-info/?biller_code=BIL099`
			)
			.then((response) => {
			...
			{page === "list" && (
				<VStack width={"full"} gap={"40px"} my={"40px"}>
					...
					<VStack width={"full"} gap={"10px"}>
						{merchantsByCountry &&
						merchantsByCountry.cables &&
						merchantsByCountry.cables.length > 0 ? (
